  ...
  const std::vector<std::string> & /*stop_interfaces*/)
{
  ...
  for (const auto & key : start_interfaces)
  {
    ...
    {
      const size_t joint_index = static_cast<size_t>(
        std::distance(info.joints.begin(), joint_it_found));
  ...
  std::vector<std::vector<double>> & values, std::vector<HandleType> & interfaces)
{
  ...
  {
    auto j = static_cast<std::size_t>(std::distance(interface_list.begin(), it));
    ...
    for (const auto & interface : component.state_interfaces)
    {
      ...
      if (it != interfaces.end())
      {
        auto index = static_cast<size_t>(
          std::distance(interfaces.begin(), it));
